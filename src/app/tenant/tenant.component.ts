...
export class TenantComponent implements OnInit, AfterViewInit {
  ...
  counts: {all: number, rejected: number, active: number, pending:number, type: number} = { all: 0, rejected: 0, active: 0, pending: 0, type: 0}
  tenants: any[] = []
  ...
  ngAfterViewInit() {
   ...
   this.filteredAndPagedResults = merge(this.form.valueChanges).pipe(
      startWith({}),
      switchMap(() => {
        return this.userS.fetchTenantList(
